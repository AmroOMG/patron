...
$sql = "SELECT * FROM quiz_teacher_student WHERE student_id = '$student_id' AND quiz_id = '$quiz_id'";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if ($row) {
    header("Location: std-quizzes.php");
} else {
    $sql2 = "SELECT * FROM subject_teacher_student WHERE student_id = '$student_id' AND subject_id = '$subject_id'";
    ...
    if ($row2) {
        ...
        $sql3 = "SELECT * FROM quiz WHERE id = '$quiz_id' AND subject_id = '$subject_id'";
        $result3 = mysqli_query($conn, $sql3);
        $row3 = mysqli_fetch_assoc($result3);
        if ($row3) {
            $sql4 = "SELECT * FROM quiz WHERE id = '$quiz_id'";
            $result4 = mysqli_query($conn, $sql4);
            $row4 = mysqli_fetch_assoc($result4);
            $quiz_title = $row4['quiz_title'];
            $teacher_id = $row4['teacher_id'];
            $duration = $row4['duration'];
            $deadline_date = $row4['deadline_date'];
            $deadline_time = $row4['deadline_time'];
            $expire = $deadline_date . '' . $deadline_time;
            if ((time() + (60 * 60 * 1)) > strtotime($expire)) {
                header("Location: std-view-subject.php?subject_id=$subject_id");
            } else {
                //هنا الامور كلها زين
                $sql5 = "INSERT INTO quiz_teacher_student (quiz_id,teacher_id,student_id,std_mark) VALUES ('$quiz_id','$teacher_id','$student_id','0')";
    ...
    <div class="quizArea">
        <div class="multipleChoiceQues">
            <div class="info">
                ...
                <h3 class="mc_quiz" style="text-transform: capitalize"> <?php echo $quiz_title; ?></h3>
...
-->
    ...
    <script>
        ...
        function disableF5(e) {
            if ((e.which || e.keyCode) == 116 || (e.ctrlKey == true && (e.key == 'r' || e.key == 'R'))) e.preventDefault();
        };
    ...
    <script>
        ...
        *****/
        ...
        function addClickedAnswerToResult(questions, presentIndex, clicked) {
            ...
            var topic_id = questions[presentIndex].topic;
            var question_id = questions[presentIndex].questionID;
            var std_answer = clicked;
            /*
        ...
        $(document).ready(function() {
            ...
            $(".resultArea").on("click", ".viewanswer", function() {
                ...
                window.location.replace("std-view-subject.php?subject_id=<?php echo $subject_id ?>");
